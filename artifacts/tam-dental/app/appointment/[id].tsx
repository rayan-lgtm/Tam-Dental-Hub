...
export default function AppointmentDetailScreen() {
  ...
  const handleCheckin = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert("Location Required", "Please enable location access to check in.");
        setCheckingIn(false);
        return;
      }
      ...
      if (dist <= CHECKIN_RADIUS_KM) {
        ...
        Alert.alert("Checked In!", "You are checked in. Please proceed to reception and let them know you're here.");
      } else {
        Alert.alert(
          "Not at Clinic",
          `Check-in is only available within ${CHECKIN_RADIUS_KM * 1000}m of the clinic. You are ${(dist * 1000).toFixed(0)}m away.`
        );
      }
    } catch {
  ...
  const handleCancel = () => {
    Alert.alert(
      "Cancel Appointment",
      "Are you sure? Cancellations within 24 hours of the appointment are not allowed.",
      [
      ...
      >
        ...
        <View style={styles.detailsCard}>
          {[
            ...
            { icon: "map-pin", label: "Location", value: apt.location },
            { icon: "hash", label: "Reference", value: apt.id },
          ].map((item) => (
        ...
        {apt.status === "upcoming" && !apt.checkedIn && (
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              Check-in is available when you are within 500m of the clinic. This helps us prepare for your arrival.
            </Text>
          </View>
        )}
        ...
        {apt.checkedIn && (
          <View style={styles.checkedInBox}>
            ...
            <Text style={styles.checkedInText}>You are checked in. Please proceed to reception.</Text>
          </View>
        )}
          ...
          <View style={styles.actionsContainer}>
            {!apt.checkedIn && (
              <Pressable style={styles.primaryBtn} onPress={handleCheckin} disabled={checkingIn}>
                ...
                <Text style={styles.primaryBtnText}>{checkingIn ? "Locating..." : "Check In at Clinic"}</Text>
              </Pressable>
            )}
            {canConfirm && (
